...
async function loadInvestors() {
    ...
    if (investorsList) {
        ...
        for (let i = 0; i < allInvestors.length; i++) {
            ...
            html += '<td class="action-buttons"><button onclick="deleteInvestor(\'' + inv.id + '\')" class="btn-danger">Delete</button></td>';
            html += '</tr>';
        }
...
async function loadBuyers() {
    ...
    if (buyersList) {
        ...
        for (let i = 0; i < allBuyers.length; i++) {
            ...
            html += '<td class="action-buttons"><button onclick="deleteBuyer(\'' + b.id + '\')" class="btn-danger">Delete</button></td>';
            html += '</tr>';
        }
...
async function loadProducts() {
    ...
    if (productsList) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            html += '<td class="action-buttons"><button onclick="deleteProduct(\'' + p.id + '\')" class="btn-danger">Delete</button></td>';
            html += '</tr>';
        }
...
async function loadInvestmentHistory() { 
    ...
    if (investmentHistory) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            html += '<td class="action-buttons"><button onclick="deleteInvestment(\'' + inv.id + '\')" class="btn-danger">Delete</button></td>';
            html += '</tr>';
        }
...
async function loadProfitHistory() { 
    ...
    if (profitHistory) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            html += '<td class="action-buttons"><button onclick="deleteProfit(\'' + p.id + '\')" class="btn-danger">Delete</button></td>';
            html += '</tr>';
        }
...
async function loadInvestmentRequests() { 
    ...
    if (requestsList) {
        if (!data || data.length === 0) {
            requestsList.innerHTML = '<tr><td colspan="6" class="empty-state">No pending requests</td></tr>';
            return;
        }
        let html = '';
        for (let i = 0; i < data.length; i++) {
            ...
            html += '<td class="action-buttons"><button onclick="approveInvestment(\'' + req.id + '\', ' + req.amount + ', \'' + req.investor_id + '\')" class="btn-success">Approve</button> <button onclick="rejectInvestment(\'' + req.id + '\')" class="btn-danger">Reject</button></td>';
            html += '</tr>';
        }
...
async function loadWithdrawals() { 
    ...
    if (withdrawalsList) {
        if (!data || data.length === 0) {
            withdrawalsList.innerHTML = '<tr><td colspan="6" class="empty-state">No withdrawals found</td></tr>';
            return;
        }
        let html = '';
        for (let i = 0; i < data.length; i++) {
            const w = data[i];
            html += '<tr>';
            html += '<td>' + escapeHtml(w.investors?.full_name||'-') + '</td>';
            ...
            html += '<td><span class="status-badge status-' + w.status + '">' + w.status + '</span></td>';
            html += '<td class="action-buttons">' + (w.status==='pending' ? '<button onclick="approveWithdrawal(\'' + w.id + '\',' + w.amount + ',\'' + w.investor_id + '\')" class="btn-success">Approve</button> <button onclick="rejectWithdrawal(\'' + w.id + '\')" class="btn-danger">Reject</button>' : '-') + '</td>';
            html += '</tr>';
        }
...
async function loadOrders() { 
    ...
    for (let i = 0; i < allOrders.length; i++) {
        ...
        html += '<tr>';
        html += '<td><strong>' + (o.id?.slice(0, 12) || '') + '...</strong><br><small style="color:#666;">' + (o.id || '') + '</small></td>';
        ...
        html += '<td><select onchange="updateOrderStatus(\'' + o.id + '\', this.value)" class="status-badge" style="padding: 5px;"><option value="pending" ' + (o.status === 'pending' ? 'selected' : '') + '>Pending</option><option value="processing" ' + (o.status === 'processing' ? 'selected' : '') + '>Processing</option><option value="shipped" ' + (o.status === 'shipped' ? 'selected' : '') + '>Shipped</option><option value="delivered" ' + (o.status === 'delivered' ? 'selected' : '') + '>Delivered</option><option value="cancelled" ' + (o.status === 'cancelled' ? 'selected' : '') + '>Cancelled</option></select></td>';
...
window.approveWithdrawal = async function(id,amount,investorId){ 
    ...
    showNotification('Approved!',false); 
    await loadWithdrawals(); 
    await loadInvestors(); 
    await loadStats(); 
};
...
window.rejectWithdrawal = async function(id){ 
    ...
    showNotification('Rejected',false); 
    await loadWithdrawals(); 
};
